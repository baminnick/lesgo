...
! NOTE: since mesh_stretch is on uv-grid, the interface is 
! always located at a uv-point
! 
! coord_int initialized as -1 on all processors, if coord 
! owns interface then coord_int = 0, if coord above interface
! then coord_int = 1, if below then remains coord_int = -1
! 
...
if (hybrid_fourier) then
     do jz = lbz, nz
        if (mesh_stretch(jz) <= hwm) then !! z-level in fourier mode
            zhyb(jz) = .true.
            coord_int = -1
...
!do jz = lbz, nz
!write(*,*) coord, jz, mesh_stretch(jz), zhyb(jz)
!enddo
!write(*,*) '----------------------'
