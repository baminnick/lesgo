...
#ifdef PPOUTPUT_SPECBUDG
!*****************************************************************************
function specbudg_compute( a, b, bb, c, d, lbz2 ) result( e )
!*****************************************************************************
! 
...
! Cuilapuj already multiplied by viscosity because divtxj was used
! Change sign of Cuilapuj because of how divtxj is set up
e(i,j,k) % tviscxx = e(i,j,k) % pdissxx - real( conjg(Culapu) + Culapu )
e(i,j,k) % tviscyy = e(i,j,k) % pdissyy - real( conjg(Cvlapv) + Cvlapv )
e(i,j,k) % tvisczz = e(i,j,k) % pdisszz - real( conjg(Cwlapw) + Cwlapw )
e(i,j,k) % tviscxy = e(i,j,k) % pdissxy - real( conjg(Cvlapu) + Culapv )
e(i,j,k) % tviscxz = e(i,j,k) % pdissxz - real( conjg(Cwlapu) + Culapw )
e(i,j,k) % tviscyz = e(i,j,k) % pdissyz - real( conjg(Cwlapv) + Cvlapw )
